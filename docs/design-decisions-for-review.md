...
| Keep Raspberry Pi as the ADS-B source tier. | Source collection is a separate failure domain from browser rendering and RTMPS delivery. | Collapse ADS-B source, rendering, and streaming into one host. | Adds one physical dependency, but makes source freshness failures easier to classify. |
