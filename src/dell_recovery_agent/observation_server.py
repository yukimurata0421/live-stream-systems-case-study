...
from cra_dell_recovery.observation import OBSERVATION_PATH
...
from cra_no_action_soak.host_status import STATUS_PATH, atomic_write_json, served_host_status_bytes, write_server_resource_status
...
from .observation_publisher import DellObservationPublisher, DellObservationPublisherConfig, _read_json

CONFIG_SCHEMA = "cra_dell_recovery.observation_server_config.v3"
V2_CONFIG_SCHEMA = "cra_dell_recovery.observation_server_config.v2"
LEGACY_CONFIG_SCHEMA = "cra_dell_recovery.observation_server_config.v1"
SAFE_REASON = re.compile(r"^[A-Z][A-Z0-9_]{2,127}$")
SERVER_RESOURCE_REFRESH_SECONDS = 5.0


def _safe_reason_code(error: Exception) -> str:
    if isinstance(error, ValueError) and SAFE_REASON.fullmatch(str(error)):
        return str(error)
    return "DELL_OBSERVATION_BUILD_FAILED"


class DellObservationHTTPSServer:
    """GET-only mTLS server with no Agent DB, command route, or effect adapter."""

    def __init__(
        self,
        publisher: DellObservationPublisher,
        ssl_context: ssl.SSLContext,
        *,
        host: str = "127.0.0.1",
        port: int = 0,
        maximum_bytes: int = 2 * 1024 * 1024,
        published_observation_file: Path | None = None,
        ...
        published_ref = published_observation_file
        ...
        class Handler(BaseHTTPRequestHandler):
            ...
            def do_GET(self) -> None:  # noqa: N802
                ...
                if self.path == STATUS_PATH and host_status_ref is not None:
                    ...
                    self.wfile.write(body)
                    return
                if self.path != OBSERVATION_PATH:
                    self._json(HTTPStatus.NOT_FOUND, {"error": "NOT_FOUND"})
                    return
                try:
                    value = publisher_ref.build()
                    body = json.dumps(value, separators=(",", ":"), sort_keys=True).encode()
                    if len(body) > maximum_ref:
                        raise ValueError("DELL_OBSERVATION_RESPONSE_TOO_LARGE")
                    if published_ref is not None:
                        try:
                            atomic_write_json(published_ref, value, mode=0o640)
                        except Exception as error:
                            persistence_failure = {
                                "schema": "cra_dell_recovery.observation_persistence_failure.v1",
                                "status": "SOAK_STATUS_SAFE_BLOCKED",
                                "error_class": type(error).__name__,
                                "control_capability_count": 0,
                                "physical_effect_count": 0,
                            ...
                            print(
                                json.dumps(persistence_failure, separators=(",", ":"), sort_keys=True),
                                file=sys.stderr,
                                flush=True,
                            )
                except Exception as error:
                    ...
                    failure = {
                        "schema": "cra_dell_recovery.observation_server_failure.v1",
                        "status": "SAFE_BLOCKED",
                        "reason_code": reason_code,
                        "error_class": type(error).__name__,
                    }
                    print(json.dumps(failure, separators=(",", ":"), sort_keys=True), file=sys.stderr, flush=True)
                    self._json(
                        HTTPStatus.SERVICE_UNAVAILABLE,
                        {
    ...
    def __enter__(self) -> DellObservationHTTPSServer:
...
def server_context(certificate: Path, private_key: Path, client_ca: Path) -> ssl.SSLContext:
    _validate_tls_path(certificate, "DELL_OBSERVATION_TLS_CERTIFICATE", secret=False)
    _validate_tls_path(private_key, "DELL_OBSERVATION_TLS_PRIVATE_KEY", secret=True)
    _validate_tls_path(client_ca, "DELL_OBSERVATION_CLIENT_CA", secret=False)
    validate_leaf_certificate(certificate, "DELL_OBSERVATION_TLS_CERTIFICATE", usage="server")
    validate_ca_certificate(client_ca, "DELL_OBSERVATION_CLIENT_CA")
...
def _load_server_config(path: Path) -> dict[str, Any]:
    ...
    if set(value) != required:
        raise ValueError("DELL_OBSERVATION_SERVER_CONFIG_FIELDS_NOT_EXACT")
    if value["schema"] not in {CONFIG_SCHEMA, V2_CONFIG_SCHEMA, LEGACY_CONFIG_SCHEMA}:
        raise ValueError("DELL_OBSERVATION_SERVER_CONFIG_SCHEMA_UNSUPPORTED")
    if value["schema"] in {CONFIG_SCHEMA, V2_CONFIG_SCHEMA}:
        require_runtime_release(str(value["server_release_id"]), "DELL_OBSERVATION_SERVER_RUNTIME_RELEASE_MISMATCH")
        for name in ("published_observation_file", "host_status_file", "server_resource_status_file"):
            if not isinstance(value[name], str) or not str(value[name]).startswith("/"):
                raise ValueError(f"DELL_OBSERVATION_SERVER_{name.upper()}_INVALID")
        if not isinstance(value["host_status_host_id"], str) or not value["host_status_host_id"].strip():
            raise ValueError("DELL_OBSERVATION_SERVER_HOST_STATUS_HOST_ID_INVALID")
    if value["schema"] == CONFIG_SCHEMA and (
        ...
        raise ValueError("DELL_OBSERVATION_SERVER_RESILIENT_HOST_STATUS_FILE_INVALID")
    try:
        listen_address = ipaddress.ip_address(str(value["listen_host"]))
    except ValueError as error:
        raise ValueError("DELL_OBSERVATION_SERVER_LISTEN_ADDRESS_INVALID") from error
    if (
        ...
        raise ValueError("DELL_OBSERVATION_SERVER_LISTEN_ADDRESS_NOT_PRIVATE_LAN")
    if not 1 <= int(value["listen_port"]) <= 65_535:
        raise ValueError("DELL_OBSERVATION_SERVER_PORT_OUT_OF_RANGE")
    if not 1024 <= int(value["maximum_response_bytes"]) <= 2 * 1024 * 1024:
        raise ValueError("DELL_OBSERVATION_SERVER_MAXIMUM_BYTES_OUT_OF_RANGE")
    return value


def main() -> None:
    parser = argparse.ArgumentParser(description="GET-only mTLS server for signed Dell observations")
    parser.add_argument("--config", type=Path, required=True)
    parser.add_argument("--check-config", action="store_true")
    args = parser.parse_args()
    value = _load_server_config(args.config)
    publisher_config = DellObservationPublisherConfig.load(Path(value["publisher_config"]))
    publisher = DellObservationPublisher(publisher_config)
    ...
    server = DellObservationHTTPSServer(
        publisher,
        context,
        ...
        published_observation_file=(
            Path(value["published_observation_file"]) if value["schema"] in {CONFIG_SCHEMA, V2_CONFIG_SCHEMA} else None
        ),
    ...
    def reload_credentials(*_: object) -> None:
        try:
            replacement = server_context(certificate, private_key, client_ca)
            generation = server.reload_tls(replacement)
            event = {
                "schema": "cra_dell_recovery.tls_reload.v1",
                "status": "RELOADED",
