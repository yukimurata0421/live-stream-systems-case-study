...
def load_exporter():
    path = Path(__file__).resolve().parents[1] / "ops" / "scripts" / "stream_v2_prometheus_exporter.py"
    spec = importlib.util.spec_from_file_location("stream_v2_prometheus_exporter", path)
...
class StreamV2PrometheusExporterTests(unittest.TestCase):
    def test_run_json_passes_v3_state_environment_to_cli(self) -> None:
        ...
        self.assertEqual(env["STREAM_V2_SOURCE_STATE_ROOT"], str(state_root))
    ...
    def test_missing_network_and_memory_snapshots_use_live_v3_evidence_fallbacks(self) -> None:
        ...
        self.assertEqual(metric_value(payload, "stream_v2_health_pass", '{window_hours="1"}'), 1.0)
        self.assertEqual(metric_value(payload, "stream_v2_current_fail", '{window_hours="1"}'), 0.0)
        self.assertEqual(metric_value(payload, "stream_v2_network_ok"), 1.0)
        self.assertEqual(metric_value(payload, "stream_v2_memory_current_ok"), 1.0)
        self.assertEqual(metric_value(payload, "stream_v2_upload_p95_mbps", '{window_hours="1"}'), 5.2)
    ...
    def test_network_socket_uses_remote_tcp_sample_when_observer_is_local_only(self) -> None:
        ...
        self.assertEqual(metric_value(payload, "stream_v2_network_ffmpeg_socket_connected"), 1.0)
        self.assertEqual(metric_value(payload, "stream_v2_network_ffmpeg_socket_notsent_bytes"), 42.0)
        self.assertEqual(metric_value(payload, "stream_v2_network_ffmpeg_socket_unacked"), 7.0)
        self.assertEqual(metric_value(payload, "stream_v2_network_ffmpeg_socket_lastsnd_ms"), 11.0)
        self.assertEqual(metric_value(payload, "stream_v2_upload_latest_mbps"), 3.05)
    ...
    def test_upload_windows_prefer_windowed_tcp_samples_over_24h_health_fields(self) -> None:
        ...
        self.assertEqual(metric_value(payload, "stream_v2_upload_p95_mbps", '{window_hours="1"}'), 3.2)
        self.assertEqual(metric_value(payload, "stream_v2_upload_max_mbps", '{window_hours="1"}'), 3.2)
        self.assertEqual(metric_value(payload, "stream_v2_upload_over_budget_seconds", '{window_hours="1"}'), 0.0)
